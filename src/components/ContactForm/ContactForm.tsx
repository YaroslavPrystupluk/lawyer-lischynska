import { forwardRef, FormEvent, useState, useImperativeHandle, useRef } from "react";
...
// Огортаємо в forwardRef, щоб підтримувати ref ззовні
    ...
    ({ onSubmitSuccess, onSubmitError, autoReset = true }, ref) => {
        ...
        const sendEmail = (e: FormEvent<HTMLFormElement>) => {
            ...
            emailjs
                ...
                .then(
                    () => {
                        setToast({ type: "success", message: "Ваш запит успішно відправлений" });
                        onSubmitSuccess?.();
                        if (autoReset) innerFormRef.current?.reset();
                    },
                    (error) => {
                        setToast({ type: "danger", message: `Ваш запит не відправлений: ${String(error)}` });
                        onSubmitError?.(error);
                    }
                )
        ...
        return (
            <>
                ...
                <form ref={innerFormRef} onSubmit={sendEmail} className="flex flex-col w-full">
                    ...
                    <textarea
                        ...
                        name="message"
                        required
                        className="mb-4 h-32 w-full resize-none rounded border border-primary px-3 py-1 text-base outline-none focus:ring-1 focus:ring-primary"
                    ></textarea>
