...
const FeedBackModal = forwardRef<HTMLFormElement, IFeedBackProps>(
    ({ handleCloseModal }, ref) => {
        const sendEmail = (e: FormEvent<HTMLFormElement>) => {
            e.preventDefault();
            if (!ref || !(ref as React.RefObject<HTMLFormElement>).current) return;

            emailjs
                .sendForm(
                    import.meta.env.VITE_SERVICE_ID,
                    import.meta.env.VITE_TEMPLATE_ID,
                    (ref as React.RefObject<HTMLFormElement>).current!,
                    {
                ...
                .then(
                    () => {
                        if ((ref as React.RefObject<HTMLFormElement>).current) {
                            (ref as React.RefObject<HTMLFormElement>).current!.reset();
                        }
                    },
                    (error) => {
                ...
                <div className="container px-5 py-24 mx-auto flex sm:flex-nowrap flex-wrap">
                    ...
                    >
                        ...
                        <p className="text-xs text-gray-500 mt-3">
                            *натискаючи «Замовити консультацію», Ви погоджуєтесь з політикою конфіденційності.
                        </p>
                    </form>
                </div>
            </section>
